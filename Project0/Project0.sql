...
CREATE OR REPLACE PROCEDURE TRANSACTION_FEE (B_ID IN BANK_ACCT.BANK_ACCT_ID%TYPE)
IS
BEGIN
   UPDATE BANK_ACCT
   SET BANK_ACCT_BALANCE = BANK_ACCT_BALANCE - 2 WHERE BANK_ACCT_ID = B_ID;
END;
...
SELECT SUM(MODIFIER) FROM TRANSACTIONS WHERE TRANSACTION_TYPE = 'deposit';
